...
- `apps/web` — Next.js 14 App Router (`@cooking/web`). Cookie-based auth. Pages include Import, Library/friends, Planner, `/cook`, Shopping, recipe detail/edit/tutorial, admin preview, settings, and auth. Library remains the authenticated landing page; primary order is Library / Planner / Cook / Shopping.
- `apps/mobile` — Expo / React Native app (`@cooking/mobile`). Bottom tabs are Library / Planner / Cook / Shopping, with Account and Import as root-level modal flows. Cook includes planned/manual setup, multi-dish focus, passive timer ring/tray, owned local notifications, screen wake, offline replay, and recipe-detail Start/Add/Open actions.
