...
`backend/app/services/weee_scraper.py` performs at most three internal attempts for one logical lookup. Every attempt creates a fresh browser context and exactly one search page, and closes both in `finally`; it extracts up to three safe unique products from search cards and never opens product-detail pages. Product URLs must be HTTPS product paths on an official `sayweee.com` or `weee.com` host (including subdomains). Only an explicit Weee no-results state is a true empty result. Navigation/hydration timeouts, challenges, unexpected routes, malformed or unsafe-only results, and browser failures are transient typed failures and are retried internally. A recovered later attempt completes the original request without surfacing an error. A confirmed empty result returns HTTP 200 with `products: []` and `expires_at: null`; exhausting all attempts returns HTTP 503 with detail code `weee_temporarily_unavailable` and `Retry-After: 3`.

Exactly one live scrape runs per backend process. Same-key callers join one cancellation-shielded job; interactive work is selected before the warmer’s next queued item (an already-running warmer item may finish). Positive products are committed through a fresh PostgreSQL session before L1 publication or waiter resolution. `backend/app/jobs/cache_warmer.py` traverses `cache_warmer_queries.py::ALL_QUERIES` serially at background priority, isolates per-query empty/failure outcomes, and force-refreshes on the 24-hour APScheduler interval. Backend startup only starts the scheduler; it does not launch a live-scrape sweep. Admins can still drive the warmer from `/preview` through `/admin/cache-refresh*`.
