...
Application shutdown stops lookup admission, settles queued callers with typed transient errors, cancels/awaits the active worker and tracked warmer, drains tracked detached work within bounded budgets, closes Playwright/browser resources, and then disposes the database engine. Every later phase still runs when an earlier cleanup fails or the shutdown caller is cancelled; failures are reported and caller cancellation is propagated afterward. Startup re-enables admission and resets fairness state. Shutdown/resource close is idempotent and cancellation is not swallowed.
