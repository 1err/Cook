...
- `navigation/` — `RootStack` swaps between auth and `MainTabs`; Import and Account are sibling root flows. `MainTabs` is Library / Planner / Cook / Shopping and `CookStack` owns `CookHome`. All `ParamList` types live in `navigation/types.ts`.
- `features/<area>/` — screens grouped by feature (`auth/`, `library/`, `planner/`, `cook/`, `shopping/`, `import/`, `profile/`). New feature work lands here, not in a flat `screens/` directory.
