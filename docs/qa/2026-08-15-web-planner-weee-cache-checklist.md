...
- [ ] Scraper/coordinator timeout tests cover hung evaluation, a combined operation-plus-cleanup deadline, independent queued-job expiry, front-door expiry, identity-token waiter settlement, browser invalidation, fixed-point detached-task/resource drains, cancellation-resistant persistence/Playwright work, no late L1/waiter publication, and next-job recovery. A timed-out physical child holds the process-local serial permit under quarantine; queued/new callers fail promptly with typed transient errors until it actually stops, peak live scrape count remains one, and normal admission resumes afterward. Production budgets remain sufficient for three normal attempts and all timeout constants remain test-patchable.
- [ ] Cache/service tests prove finite age `0` and `86,399` seconds are fresh, future/NaN/infinite/invalid timestamps and age `86,400` seconds are rejected in L1, single L2, and batch L2, no stale fallback is displayed after refresh failure, same-key callers join one job, interactive work normally precedes background work, bounded fairness advances a waiting warmer under sustained interactive arrivals, and a positive row commits before publication. Commit-boundary tests invalidate ownership during a cancellation-resistant commit, prove the real commit outcome is awaited without a false rollback, block late L1/waiter publication, and prove equal/older database generations cannot overwrite the winner.
