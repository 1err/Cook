...
- [ ] Scraper trust-boundary tests prove the final URL requires HTTPS, no credentials or non-443 port, an exact/subdomain official host, the exact language search path, and exactly one correctly decoded expected `keyword`. Real Chromium DOM tests prove challenge signals dominate mixed product/empty markup, body text alone is not an empty result, a visible specific no-result element is empty, and real product cards extract.
- [ ] Scraper/coordinator timeout tests cover hung evaluation, bounded cleanup, queue/front-door expiry, typed waiter settlement, browser invalidation, and next-job recovery. Production budgets remain sufficient for three normal attempts and all timeout constants remain test-patchable.
- [ ] Cache/service tests prove age `86,399` seconds is fresh, age `86,400` seconds is expired, no stale fallback is displayed after refresh failure, same-key callers join one job, interactive work normally precedes background work, bounded fairness advances a waiting warmer under sustained interactive arrivals, and a positive row commits before publication.
- [ ] L1 tests prove the 256-entry TTL/LRU bound, touch-before-eviction behavior, and opportunistic global expired pruning. L2/L1/restart tests prove one shared safe normalization preserves `Rice 1 lb` versus `Rice 2 lb`, rejects unsafe and duplicate rows, preserves order, caps product choices at three, and publishes exactly what was persisted.
- [ ] Route/session tests prove Unicode-whitespace-only GET queries return 4xx, unsupported-store validation runs first, and authenticated cold misses release the short request read session before waiting in the live queue without closing unrelated caller-owned sessions.
...
- [ ] Web tests prove malformed `fresh` batch rows always fall back to serial GET, rejected/malformed batch fallbacks enqueue all current priorities before pumping, and persisted canonical query metadata retains the first cleaned spelling across aliases, hydration, Retry, reload, and exact expiry.
- [ ] Warmer tests prove the 24-hour scheduled run is forced and serial, per-query failures do not stop later queries, scheduled/manual starts share one atomic tracked run in both orders, and backend startup schedules the job without starting a live-scrape sweep.
- [ ] Lifecycle tests prove startup re-enables admission and shutdown deterministically stops admission, settles active/queued requests, awaits worker and warmer cancellation, closes browser/Playwright resources idempotently within budget, and disposes the database engine.
