...
- [ ] Scraper trust-boundary tests prove the final URL requires HTTPS, no credentials or non-443 port, an exact/subdomain official host, the exact language search path (optional trailing slash), and exactly one correctly decoded expected `keyword`, while allowing unrelated tracking parameters. Real Chromium DOM tests prove challenge signals dominate mixed markup, search-card classification and extraction share one scope, recommendation/carousel products are excluded, conflicting scoped product plus no-result signals remain untrusted, body text alone is not empty, and realistic search cards extract.
- [ ] Scraper/coordinator timeout tests cover hung evaluation, a combined operation-plus-cleanup deadline, queue/front-door expiry, typed waiter settlement, browser invalidation, explicit detached-task drains, cancellation-resistant persistence/Playwright work, no late L1/waiter publication, and next-job recovery. Production budgets remain sufficient for three normal attempts and all timeout constants remain test-patchable.
...
- [ ] Web tests prove malformed `fresh` batch rows always fall back to serial GET, rejected/malformed batch fallbacks enqueue all current priorities before pumping, live/batch/persisted positives share the 1–120-character/safe-URL/whitespace/dedupe/order/cap-three contract (`姜` remains valid), and canonical query metadata retains the first cleaned spelling across aliases, hydration, Retry, reload, and exact expiry while rotating on generation changes.
- [ ] Warmer tests prove the 24-hour scheduled run is forced and serial, per-query failures do not stop later queries, scheduled/manual starts share one atomic tracked run in both orders, unexpected tracked-task exceptions are consumed/logged, caller cancellation is propagated, and backend startup schedules the job without starting a live-scrape sweep.
- [ ] Lifecycle tests prove startup re-enables admission and resets fairness, while shutdown deterministically stops admission, settles active/queued requests, retains cancellation-resistant worker ownership, drains detached work, closes browser/Playwright resources idempotently within budget, runs browser and database cleanup after earlier failures/cancellation, and then propagates the caller outcome.
