...
- [ ] Scraper/coordinator timeout tests cover hung evaluation, a combined operation-plus-cleanup deadline, independent queued-job expiry, front-door expiry, identity-token waiter settlement, browser invalidation, fixed-point detached-task/resource drains, cancellation-resistant persistence/Playwright work, no late L1/waiter publication, and next-job recovery. Ordinary or repeated cancellation cannot interrupt the exact-once waiter-token release; the final waiter invalidates/cancels an unfinished flight while a remaining same-key waiter preserves it. A child that catches final-waiter cancellation and raises an ordinary exception still reaches physical quiescence and clears only its own quarantine. A timed-out physical child holds the process-local serial permit under quarantine; queued/new callers fail promptly with typed transient errors until it actually stops, peak live scrape count remains one, and normal admission resumes afterward. Late page/context cleanup failures are surfaced and retire their exact browser before retry. A combined child-close plus connected-browser-close failure retains exact resource ownership, starts no second browser/attempt, returns typed transient unavailability, and a later retirement retry recovers idempotently before the next launch. Failed Chromium launch plus Playwright `stop()` exception, timeout, or caller cancellation retains the exact driver owner, fences relaunch, and recovers only after confirmed retirement. Production budgets remain sufficient for three normal attempts and all timeout constants remain test-patchable.
- [ ] Cache/service tests prove finite age `0` and `86,399` seconds are fresh, future/NaN/infinite/invalid timestamps and age `86,400` seconds are rejected in L1, single L2, and batch L2, no stale fallback is displayed after refresh failure, same-key callers join one job, interactive work normally precedes background work, bounded fairness advances a waiting warmer under sustained interactive arrivals, and stale fairness state resets at idle/backlog removal. Single and batch L2 each use one execute with a shared materialized PostgreSQL observation. Authoritative DB age is translated onto local receipt/monotonic time, so a valid winner ahead of the host clock succeeds without raw HTTP 500; the pre-execute anchor is retained through validation and L1 insertion, so neither DB response latency nor an arbitrary handoff delay can extend exact-boundary TTL or public expiry. Commit-boundary tests prove pre-commit failures roll back, invalidate ownership during a cancellation-resistant commit, await the real commit outcome without a false rollback after invocation begins, and block late L1/waiter publication. PostgreSQL-dialect statement tests prove each positive candidate uses one atomic `INSERT ... ON CONFLICT DO UPDATE` with volatile evaluation-time `clock_timestamp()` for candidate generation, winner observation, and future-poison recovery; a conflict blocked behind a concurrent winner is not evaluated against frozen transaction-start time. Equal and newer valid incumbents remain authoritative. Candidate wins return data/timestamp/observation through `RETURNING`, while a conditional conflict loser is read with a fresh observation in the same transaction after the conflict wait, committed without rollback, and published identically through L1 and the response. A +100-year future-poisoned row is replaceable without treating normal host/DB offset as displayable future skew.
